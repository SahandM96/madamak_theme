...
final class DUPX_Paramas_Descriptors
{
    ...
    public static function initPriorityParams(&$params)
    {
        ...
        $oldMainPath = DupProSnapLibIou::safePathUntrailingslashit($archive_config->getRealValue('homePath'));
        ...
        $oldContentPath     = DupProSnapLibIou::safePathUntrailingslashit($archive_config->getRealValue('contentPath'));
        $oldUploadsBasePath = DupProSnapLibIou::safePathUntrailingslashit($archive_config->getRealValue('uploadBasePath'));
        $oldPluginsPath     = rtrim($archive_config->getDefineValue('WP_PLUGIN_DIR', $oldMainPath.'/wp-content/plugins'), '/');
        $oldPluginsUrl      = rtrim($archive_config->getDefineValue('WP_PLUGIN_URL', $oldHomeUrl.'/wp-content/plugins'), '/');
        $oldMuPluginsPath   = rtrim($archive_config->getDefineValue('WPMU_PLUGIN_DIR', $oldMainPath.'/wp-content/mu-plugins'), '/');
        $oldMuPluginsUrl    = rtrim($archive_config->getDefineValue('WPMU_PLUGIN_URL', $oldHomeUrl.'/wp-content/mu-plugins'), '/');
...
NOTE
    ...
    public static function initScanParams(&$params)
    {
        ...
        foreach ($tables as $table => $tableInfo) {
            $tablesOptions[] = new DUPX_Param_item_form_option($table, $table.' ('.$tableInfo->rows.')');
        }
