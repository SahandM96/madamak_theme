...
if (is_admin() === true) {
    ...
    {        
        ...
        if (DUPLICATOR_PRO_VERSION != $current_version || $force)
        {
            ...
            require_once($root_path.'/wp-admin/includes/upgrade.php');
        ...
        function duplicator_pro_unhook_third_party_assets($hook)
        {
            ...
            */
            if (strpos($hook, 'duplicator-pro') !== false) {
                $global = DUP_PRO_Global_Entity::get_instance();
                $assets = array();
                if ($global->unhook_third_party_css)  $assets['styles'] = wp_styles();
                if ($global->unhook_third_party_js)  $assets['scripts'] = wp_scripts();
