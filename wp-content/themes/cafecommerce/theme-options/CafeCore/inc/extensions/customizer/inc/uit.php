...
class cafe_sdk {
	...
	public function admin_menu() {
		add_submenu_page(
			$this->parent_slug,
			__('فعال سازی قالب کافه کامرس', $this->text_domain),
			__('فعال سازی قالب کافه کامرس', $this->text_domain),
			'manage_options',
			$this->slug,
     ...
     * If you want add an interrupt in your plugin or theme simply can use this static method: cafe_sdk::is_activated
...
add_action('init', 'cafe_commerce_init');
/**
 * Initialize function for class and hook it to wordpress init action
 */
function cafe_commerce_init() {
	$settings = [
		'name'          => 'cafecommerce',
		'slug'          => 'cafe_commerce_license_register',
		...
		'product_token' => '05ee2d8c-d893-4085-9228-39fe25fe1ab1', // Get it from here: https://zhaket.com/dashboard/licenses/
		'option_name'   => 'cafe_commerce_register_settings'
	];
	cafe_sdk::instance($settings);
}